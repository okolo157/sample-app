...
const CometChatLogin = () => {
  ...
  function hasCredentials() {
    const appID: string =
      localStorage.getItem("appId") || COMETCHAT_CONSTANTS.APP_ID; // Use the latest appId if available
    const region: string =
      localStorage.getItem("region") || COMETCHAT_CONSTANTS.REGION; // Default to 'us' if region is not found
    const authKey: string =
      localStorage.getItem("authKey") || COMETCHAT_CONSTANTS.AUTH_KEY; // Default authKey if not found
    ...
    return (
      <>
        <div
          key={uid}
          onClick={() => login(uid)}
          className={`cometchat-login__user ${
            selectedUid === uid ? "cometchat-login__user-selected " : ""
          }`}
        >
          {selectedUid === uid ? (
            <>
  ...
  return (
    <div className="cometchat-login__container">
      ...
      <div className="cometchat-login__content">
        ...
        {/* <div
          className="cometchat-login__divider-section"
          style={{ display: "flex" }}
        >
        ...
        <div className="cometchat-login__custom-login">
          ...
          >
            <div className="cometchat-login__input-group">
              <label
                className="input-label cometchat-login__input-label"
                htmlFor=""
              >
                Your UID
              </label>
            ...
            <div className="cometchat-login__note">
              Change{" "}
              <span
                className="cometchat-login__note-link"
